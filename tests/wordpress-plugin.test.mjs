...
test("WordPress eklentisi konum ve CSP ayarlarını güvenli biçimde sınırlar", async () => {
  ...
  assert.match(php, /Version:\s+1\.0\.17/);
...
test("WordPress varlıkları widget 1.0.17 sürümünü ve yerel CSP fontlarını kullanır", async () => {
  ...
  assert.match(standard, /IMU Web Accessibility v1\.0\.17/);
  assert.match(csp, /IMU Web Accessibility v1\.0\.17/);
...
test("ana README standart, CSP ve WordPress kurulumlarını belgeler", async () => {
  ...
  assert.match(readme, /Sürüm: 1\.0\.17/);
  assert.match(readme, /imu-web-accessibility-1\.0\.17\.zip/);
